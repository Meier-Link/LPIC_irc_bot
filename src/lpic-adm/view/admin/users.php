...
<div class="table">
  ...
  foreach($controller->data['users'] as $u)
  {
    ?>
    <div class="line">
      <form method="POST" action="/admin/users/<?php echo $u->u_id(); ?>" />
        <div class="cell" style="width:30%;"><?php echo $u->u_pseudo(); ?></div>
        <div class="cell" style="width:30%;">
          <input type="radio" id="is_not_manager" name="user[is_manager]" 
            value="0" <?php if($u->u_is_manager() == 0) { echo "checked"; } ?> />
          &nbsp;<label for="is_not_manager">False</label>
          <input type="radio" id="is_manager" name="user[is_manager]" 
            value="1" <?php if($u->u_is_manager() == 1) { echo "checked"; } ?> />
